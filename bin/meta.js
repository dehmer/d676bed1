...
const legacy = ({ standard }) => xs => [
  `symbol+${standard}:${props([2, 3, 4, 5, 6], xs)}*****`,
  {
    hierarchy: xs[1].trim(),
    name: xs[7],
...
const current = ({ standard, symbolset }) => xs => [
  `symbol+${standard}:100*${symbolset}*000${props([4], xs)}0000`,
  {
    entity: xs[0],
    type: xs[1],
    subtype: xs[2],
...
const modifier = ({ standard, symbolset, sector }) => xs => [
  `modifier+${standard}:${symbolset}/${sector}/${xs[2]}`,
  {
    modifier: xs[0],
    category: xs[1]
  }
...
const isLegacy = ({ standard }) => ['2525+b', '2525+c', 'app6+b'].includes(standard)
